...
int searcher_module(lua_State* L) {
	std::string name = luaL_checkstring(L, 1);
	std::string path = name[0] == '/' ? (name + "/module.lua") : ("/lib/" + name + "module.lua");
...
lua_CFunction tos::searcher::tossearcher_module(const tos_global& os)
{
	setup(os);
